    ...
    $("#btnImport").click(function () {
        jp.open({
            ...
            btn1: function (index, layero) {
                window.location = '${ctx}/oms/order/omChainContractLogisticsOrder/import/template';
            },
...
function initTable() {
    $('#omChainHeaderTable').bootstrapTable({
        ...
        url: "${ctx}/oms/order/omChainContractLogisticsOrder/data",
        ...
        columns: [{
            ...
            sortable: true,
            formatter: function (value, row, index) {
                return jp.getDictLabel(${fns:toJson(fns:getDictList('OMS_BUSINESS_ORDER_TYPE_CONTRACT'))}, value, "-");
            }
        }, {
            field: 'ownerName',
            title: '货主',
...
function deleteAll() {
    ...
    jp.confirm('确认要删除该合同物流订单记录吗？', function () {
        jp.loading();
        jp.get("${ctx}/oms/order/omChainContractLogisticsOrder/deleteAll?ids=" + getIdSelections(), function (data) {
            if (data.success) {
                $('#omChainHeaderTable').bootstrapTable('refresh');
                jp.success(data.msg);
            } else {
...
function add() {
    jp.openDialog('新增合同物流订单', "${ctx}/oms/order/omChainContractLogisticsOrder/form", '90%', '90%', $('#omChainHeaderTable'));
}
...
function edit(id, status) {//没有权限时，不显示确定按钮
    ...
    <shiro:lacksPermission name="order:omChainContractLogisticsOrder:edit">
        jp.openDialogView('查看合同物流订单', "${ctx}/oms/order/omChainContractLogisticsOrder/form?id=" + id, '90%', '90%', $('#omChainHeaderTable'));
    </shiro:lacksPermission>
    <shiro:hasPermission name="order:omChainContractLogisticsOrder:edit">
        jp.openDialog('编辑合同物流订单', "${ctx}/oms/order/omChainContractLogisticsOrder/form?id=" + id, '90%', '90%', $('#omChainHeaderTable'));
    </shiro:hasPermission>
}
...
function view(id) {
    ...
    <shiro:hasPermission name="order:omChainContractLogisticsOrder:view">
        jp.openDialogView('查看合同物流订单', "${ctx}/oms/order/omChainContractLogisticsOrder/form?id=" + id, '90%', '90%', $('#omChainHeaderTable'));
    </shiro:hasPermission>
}
...
function audit() {
    ...
    jp.loading();
    jp.get("${ctx}/oms/order/omChainContractLogisticsOrder/audit?ids=" + getIdSelections(), function (data) {
        if (data.success) {
            $('#omChainHeaderTable').bootstrapTable('refresh');
            jp.success(data.msg);
        } else {
...
function cancelAudit() {
    ...
    jp.loading();
    jp.get("${ctx}/oms/order/omChainContractLogisticsOrder/cancelAudit?ids=" + getIdSelections(), function (data) {
        if (data.success) {
            $('#omChainHeaderTable').bootstrapTable('refresh');
            jp.success(data.msg);
        } else {
