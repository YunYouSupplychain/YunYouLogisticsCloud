    ...
    $("#btnImport").click(function () {
        jp.open({
            ...
            btn1: function (index, layero) {
                window.location = '${ctx}/oms/order/omChainB2CGroupPurchaseOrder/import/template';
            },
...
function initTable() {
    $('#omChainHeaderTable').bootstrapTable({
        ...
        url: "${ctx}/oms/order/omChainB2CGroupPurchaseOrder/data",
        ...
        columns: [{
            ...
            sortable: true,
            formatter: function (value, row, index) {
                return jp.getDictLabel(${fns:toJson(fns:getDictList('OMS_BUSINESS_ORDER_TYPE_B2C_GP'))}, value, "-");
            }
        }, {
            field: 'ownerName',
            title: '货主',
...
function deleteAll() {
    ...
    jp.confirm('确认要删除该B2C团购订单记录吗？', function () {
        jp.loading();
        jp.get("${ctx}/oms/order/omChainB2CGroupPurchaseOrder/deleteAll?ids=" + getIdSelections(), function (data) {
            if (data.success) {
                $('#omChainHeaderTable').bootstrapTable('refresh');
                jp.success(data.msg);
            } else {
...
function add() {
    jp.openDialog('新增B2C团购订单', "${ctx}/oms/order/omChainB2CGroupPurchaseOrder/form", '90%', '90%', $('#omChainHeaderTable'));
}
...
function edit(id, status) {//没有权限时，不显示确定按钮
    ...
    <shiro:lacksPermission name="order:omChainB2CGroupPurchaseOrder:edit">
        jp.openDialogView('查看B2C团购订单', "${ctx}/oms/order/omChainB2CGroupPurchaseOrder/form?id=" + id, '90%', '90%', $('#omChainHeaderTable'));
    </shiro:lacksPermission>
    <shiro:hasPermission name="order:omChainB2CGroupPurchaseOrder:edit">
        jp.openDialog('编辑B2C团购订单', "${ctx}/oms/order/omChainB2CGroupPurchaseOrder/form?id=" + id, '90%', '90%', $('#omChainHeaderTable'));
    </shiro:hasPermission>
}
...
function view(id) {
    ...
    <shiro:hasPermission name="order:omChainB2CGroupPurchaseOrder:view">
        jp.openDialogView('查看B2C团购订单', "${ctx}/oms/order/omChainB2CGroupPurchaseOrder/form?id=" + id, '90%', '90%', $('#omChainHeaderTable'));
    </shiro:hasPermission>
}
...
function audit() {
    ...
    jp.loading();
    jp.get("${ctx}/oms/order/omChainB2CGroupPurchaseOrder/audit?ids=" + getIdSelections(), function (data) {
        if (data.success) {
            $('#omChainHeaderTable').bootstrapTable('refresh');
            jp.success(data.msg);
        } else {
...
function cancelAudit() {
    ...
    jp.loading();
    jp.get("${ctx}/oms/order/omChainB2CGroupPurchaseOrder/cancelAudit?ids=" + getIdSelections(), function (data) {
        if (data.success) {
            $('#omChainHeaderTable').bootstrapTable('refresh');
            jp.success(data.msg);
        } else {
