    ...
    $("#btnImport").click(function () {
        jp.open({
            ...
            btn1: function (index, layero) {
                window.location = '${ctx}/oms/order/omChainSupReturnOrder/import/template';
            },
...
function initTable() {
    $('#omChainHeaderTable').bootstrapTable({
        ...
        url: "${ctx}/oms/order/omChainSupReturnOrder/data",
        ...
        columns: [{
            ...
            sortable: true,
            formatter: function (value, row, index) {
                return jp.getDictLabel(${fns:toJson(fns:getDictList('OMS_BUSINESS_ORDER_TYPE_SUP_MRB'))}, value, "-");
            }
        }, {
            field: 'ownerName',
            title: '货主',
...
function deleteAll() {
    ...
    jp.confirm('确认要删除该供应商退货订单记录吗？', function () {
        jp.loading();
        jp.get("${ctx}/oms/order/omChainSupReturnOrder/deleteAll?ids=" + getIdSelections(), function (data) {
            if (data.success) {
                $('#omChainHeaderTable').bootstrapTable('refresh');
                jp.success(data.msg);
            } else {
...
function add() {
    jp.openDialog('新增供应商退货订单', "${ctx}/oms/order/omChainSupReturnOrder/form", '90%', '90%', $('#omChainHeaderTable'));
}
...
function edit(id, status) {//没有权限时，不显示确定按钮
    ...
    <shiro:lacksPermission name="order:omChainSupReturnOrder:edit">
        jp.openDialogView('查看供应商退货订单', "${ctx}/oms/order/omChainSupReturnOrder/form?id=" + id, '90%', '90%', $('#omChainHeaderTable'));
    </shiro:lacksPermission>
    <shiro:hasPermission name="order:omChainSupReturnOrder:edit">
        jp.openDialog('编辑供应商退货订单', "${ctx}/oms/order/omChainSupReturnOrder/form?id=" + id, '90%', '90%', $('#omChainHeaderTable'));
    </shiro:hasPermission>
}
...
function view(id) {
    ...
    <shiro:hasPermission name="order:omChainSupReturnOrder:view">
        jp.openDialogView('查看供应商退货订单', "${ctx}/oms/order/omChainSupReturnOrder/form?id=" + id, '90%', '90%', $('#omChainHeaderTable'));
    </shiro:hasPermission>
}
...
function audit() {
    ...
    jp.loading();
    jp.get("${ctx}/oms/order/omChainSupReturnOrder/audit?ids=" + getIdSelections(), function (data) {
        if (data.success) {
            $('#omChainHeaderTable').bootstrapTable('refresh');
            jp.success(data.msg);
        } else {
...
function cancelAudit() {
    ...
    jp.loading();
    jp.get("${ctx}/oms/order/omChainSupReturnOrder/cancelAudit?ids=" + getIdSelections(), function (data) {
        if (data.success) {
            $('#omChainHeaderTable').bootstrapTable('refresh');
            jp.success(data.msg);
        } else {
