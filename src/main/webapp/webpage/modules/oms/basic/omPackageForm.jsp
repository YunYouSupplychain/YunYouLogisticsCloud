    ...
    <script type="text/javascript">
        ...
        function validateHeaderForm() {
            validateForm = $("#inputForm").validate({
                submitHandler: function (form) {
                    jp.post("${ctx}/oms/basic/omPackage/save", $('#inputForm').bq_serialize(), function (data) {
                        if (data.success) {
        ...
        function init() {
            if (!$('#id').val()) {
                ...
                $.get("${ctx}/oms/basic/omPackageRelation/initialList", function (data) {
                ...
                $.get("${ctx}/oms/basic/omPackageRelation/data?" + params, function (data) {
                    for (var i = 0; i < data.length; i++) {
                        addRow('#packageDetailList', i, packageDetailTpl, data[i])
                    }
                });
        ...
        function addRow(list, idx, tpl, row) {
            ...
            $(list + idx + '_cdprIsReview').prop('checked', row.cdprIsReview === 'Y').val(row.cdprIsReview);
        }
...
<body>
<form:form id="inputForm" modelAttribute="omPackage" method="post" class="form-horizontal">
    <form:hidden path="id"/>
...
<tr id="packageDetailList{{idx}}">
    ...
    <td>
        <input id="packageDetailList{{idx}}_cdprQuantity" name="packageDetailList[{{idx}}].cdprQuantity" type="text" value="{{row.cdprQuantity}}" class="form-control" onkeyup="value=value.replace(/[^\d]/g,'')"/>
    </td>
    <td width="100px">
    ...
    <td width="80px">
        <input id="packageDetailList{{idx}}_cdprIsReview" name="packageDetailList[{{idx}}].cdprIsReview" type="checkbox" class="myCheckbox"/>
    </td>
</tr>//-->
</script>
<script type="text/javascript">
