...
<script>
var tmSpareSoDetailRowIdx = 0, tmSpareSoDetailTpl;
$(document).ready(function () {
    tmSpareSoDetailTpl = $("#tmSpareSoDetailTpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g, "");
    $("#orderTime").datetimepicker({format: "YYYY-MM-DD HH:mm:ss"});
    $("#tmSpareSoDetailTable").find("input[name='btSelectAll']").on('click', function () {
...
function save() {
    ...
    jp.post("${ctx}/tms/spare/so/save", params, function (data) {
        if (data.success) {
            window.location = "${ctx}/tms/spare/so/form?id=" + data.body.entity.id;
        } else {
            jp.error(data.msg);
        }
    });
...
function initDetail() {
    tmSpareSoDetailRowIdx = 0;
    $('#tmSpareSoDetailList').empty();
    jp.post("${ctx}/tms/spare/so/detail/data", {spareSoNo: $('#spareSoNo').val(), orgId: $('#orgId').val()}, function (rows) {
...
function getNewLineNo() {
    ...
    $.map($('#tmSpareSoDetailList .lineNo'), function (obj) {
...
function addDetail(row) {
    if (row === undefined) {
        row = {spareSoNo: $('#spareSoNo').val(), lineNo: getNewLineNo(), orgId: $('#orgId').val(), baseOrgId: $('#baseOrgId').val(), recVer: 0};
    }
    addRow('#tmSpareSoDetailList', tmSpareSoDetailRowIdx, tmSpareSoDetailTpl, row);
    tmSpareSoDetailRowIdx = tmSpareSoDetailRowIdx + 1;
}

function saveDetail() {
    jp.loading();
    var validator = bq.tableValidate('#tmSpareSoDetailForm');
    if (!validator.isSuccess){
        jp.warning(validator.msg);
        return;
    }
    var objs = bq.openDisabled("#tmSpareSoDetailForm");
    var params = $('#tmSpareSoDetailForm').serialize();
    bq.closeDisabled(objs);
    jp.post("${ctx}/tms/spare/so/detail/save", params, function (data) {
        initDetail();
        if (data.success) {
            jp.success(data.msg);
        } else {
...
function removeDetail() {
    delRow('#tmSpareSoDetailList', "${ctx}/tms/spare/so/detail/deleteAll");
}

function initScanInfoTable() {
    $('#tmSpareSoScanInfoTable').bootstrapTable({
        ...
        url: "${ctx}/tms/spare/so/scan/data",
        queryParams: function (params) {
            var searchParam = $("#searchForm").serializeJSON();
            searchParam.spareSoNo = $('#spareSoNo').val();
