...
function initTable() {
    ...
    $table.bootstrapTable({
        ...
        url: "${ctx}/tms/basic/tmFitting/data",
...
function deleteAll() {
	jp.confirm('确认要删除该配件信息记录吗？', function () {
		jp.loading();
		jp.get("${ctx}/tms/basic/tmFitting/deleteAll?ids=" + getIdSelections(), function (data) {
			if (data.success) {
				$('#tmFittingTable').bootstrapTable('refresh');
				jp.success(data.msg);
			} else {
...
function add() {
	jp.openDialog('配件信息', "${ctx}/tms/basic/tmFitting/form", '100%', '100%', $('#tmFittingTable'));
}
...
function edit(id) {
	...
	jp.openDialog('配件信息', "${ctx}/tms/basic/tmFitting/form?id=" + id, '100%', '100%', $('#tmFittingTable'));
}

function enable(flag) {
	jp.loading();
	jp.post("${ctx}/tms/basic/tmFitting/enable?ids=" + getIdSelections() + "&flag=" + flag, {}, function (data) {
		if (data.success) {
			$('#tmFittingTable').bootstrapTable('refresh');
			jp.success(data.msg);
		} else {
