...
function initTable() {
    ...
    $table.bootstrapTable({
        ...
        url: "${ctx}/tms/basic/tmBusinessRoute/data",
...
function deleteAll() {
	jp.confirm('确认要删除该业务路线信息记录吗？', function () {
		jp.loading();
		jp.get("${ctx}/tms/basic/tmBusinessRoute/deleteAll?ids=" + getIdSelections(), function (data) {
			if (data.success) {
				$('#tmBusinessRouteTable').bootstrapTable('refresh');
				jp.success(data.msg);
			} else {
...
function add() {
	jp.openDialog('业务路线信息', "${ctx}/tms/basic/tmBusinessRoute/form", '600px', '300px', $('#tmBusinessRouteTable'));
}
...
function edit(id) {
	...
	jp.openDialog('业务路线信息', "${ctx}/tms/basic/tmBusinessRoute/form?id=" + id, '600px', '300px', $('#tmBusinessRouteTable'));
}
