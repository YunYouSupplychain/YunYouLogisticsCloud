...
$(document).ready(function () {
	...
	$('#tmDriverQualificationTable').bootstrapTable({
		...
		url: "${ctx}/tms/basic/tmDriverQualification/data",
	...
	$('#tmDriverQualificationTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
		...
		var length = $("#tmDriverQualificationTable").bootstrapTable('getSelections').length;
		$('#driverQualification_remove').attr('disabled', !length);
		$('#driverQualification_edit').attr('disabled', length !== 1);
	});
});
...
function doSubmit($table, index) {
	...
	jp.post("${ctx}/tms/basic/tmDriver/save", $('#inputForm').serialize(), function (data) {
		if (data.success) {
			jp.success(data.msg);
...
function driverQualificationModal(row) {
	...
	$("#driverQualification_orgId").val($("#orgId").val());
...
function saveDriverQualification() {
	...
	jp.post("${ctx}/tms/basic/tmDriverQualification/save", $('#tmDriverQualificationForm').serialize(), function (data) {
		if (data.success) {
			jp.success(data.msg);
...
function delDriverQualification() {
	jp.confirm('确认要删除该资质记录吗？', function () {
		jp.loading();
		jp.get("${ctx}/tms/basic/tmDriverQualification/deleteAll?ids=" + getDriverQualificationIdSelections(), function (data) {
			if (data.success) {
				$('#tmDriverQualificationTable').bootstrapTable('refresh');
				jp.success(data.msg);
			} else {
