...
$(document).ready(function () {
    ...
    $('#shipTimeFm').datetimepicker({format: "YYYY-MM-DD 00:00:00"});
    $('#shipTimeTo').datetimepicker({format: "YYYY-MM-DD 23:59:59"});
    ...
    $("#search").click("click", function () {// 绑定查询按扭
        ...
        $('#bmsOutboundDataTable').bootstrapTable('refresh', {url: '${ctx}/bms/business/bmsOutboundData/data'});
    });
    ...
    $("#btnImport").click(function () {
        bq.importExcel('${ctx}/bms/business/bmsOutboundData/import');
    });
});

function initTable() {
    $('#bmsOutboundDataTable').bootstrapTable({
        ...
        columns: [{
            ...
            formatter: function (value, row, index) {
                return jp.getDictLabel(${fns:toJson(fns:getDictList('BMS_OUTBOUND_TYPE'))}, value, value);
            }
        }, {
            field: 'businessType',
            title: '业务订单类型',
        ...
        var length = $('#bmsOutboundDataTable').bootstrapTable('getSelections').length;
...
function getIdSelections() {
    return $.map($("#bmsOutboundDataTable").bootstrapTable('getSelections'), function (row) {
        return row.id
    });
}

function deleteAll() {
    jp.confirm('确认要删除该出库数据记录吗？', function () {
        jp.loading();
        jp.get("${ctx}/bms/business/bmsOutboundData/deleteAll?ids=" + getIdSelections(), function (data) {
            if (data.success) {
                $('#bmsOutboundDataTable').bootstrapTable('refresh');
                jp.success(data.msg);
            } else {
                jp.error(data.msg);
            }
...
function pullData() {
    ...
    $.ajax({
        type: "POST",
        url: "${ctx}/bms/business/bmsOutboundData/pullData",
...
function isFee(flag) {
    jp.confirm('是否确认操作？', function(){
        jp.loading("正在处理中……");
        jp.get("${ctx}/bms/business/bmsOutboundData/isFee?ids=" + getIdSelections() + "&isFee=" + flag, function (data) {
            if (data.success) {
                $('#bmsOutboundDataTable').bootstrapTable('refresh');
                jp.success(data.msg);
            } else {
...
function exportExcel() {
    ...
    bq.exportExcelNew("${ctx}/bms/business/bmsOutboundData/export", "出库数据", searchParam)
}
