...
$(document).ready(function () {
    ...
    $("#search").click("click", function () {// 绑定查询按扭
        ...
        $('#bmsWaybillDataTable').bootstrapTable('refresh', {url: "${ctx}/bms/business/bmsWaybillData/data"});
    });
    ...
    $("#btnImport").click(function () {
        bq.importExcel('${ctx}/bms/business/bmsWaybillData/import');
    });
});

function initTable() {
    $('#bmsWaybillDataTable').bootstrapTable({
        ...
        var length = $('#bmsWaybillDataTable').bootstrapTable('getSelections').length;
...
function getIdSelections() {
    return $.map($("#bmsWaybillDataTable").bootstrapTable('getSelections'), function (row) {
        return row.id
    });
}

function deleteAll() {
    jp.confirm('确认要删除该运单数据记录吗？', function () {
        jp.loading();
        jp.get("${ctx}/bms/business/bmsWaybillData/deleteAll?ids=" + getIdSelections(), function (data) {
            $('#bmsWaybillDataTable').bootstrapTable('refresh');
            if (data.success) {
                jp.success(data.msg);
            } else {
                jp.error(data.msg);
...
function pull() {
    ...
    $("#pullDataForm").find("#orgId").eq(0).val(jp.getCurrentOrg().orgId);
}
...
function pullData() {
    ...
    $.ajax({
        type: "POST",
        url: "${ctx}/bms/business/bmsWaybillData/pullData",
...
function isFee(flag) {
    jp.confirm('是否确认操作？', function () {
        jp.loading("正在处理中……");
        jp.get("${ctx}/bms/business/bmsWaybillData/isFee?ids=" + getIdSelections() + "&isFee=" + flag, function (data) {
            if (data.success) {
                $('#bmsWaybillDataTable').bootstrapTable('refresh');
                jp.success(data.msg);
            } else {
...
function exportExcel() {
    ...
    bq.exportExcelNew("${ctx}/bms/business/bmsWaybillData/export", "运单数据", searchParam)
}
