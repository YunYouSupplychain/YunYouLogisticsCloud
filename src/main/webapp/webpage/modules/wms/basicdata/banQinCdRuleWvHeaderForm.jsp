    ...
    <script type="text/javascript">
        ...
        function save() {
            ...
            if (isValidate.isSuccess) {
                jp.loading();
                jp.post("${ctx}/wms/basicdata/banQinCdRuleWvHeader/save", $('#inputForm').serialize(), function (data) {
                    if (data.success) {
                        jp.success(data.msg);
                        window.location = "${ctx}/wms/basicdata/banQinCdRuleWvHeader/form?id=" + data.body.entity.id;
                    } else {
                        jp.bqError(data.msg);
                    }
                })
        ...
        function initCdRuleWvDetailTable() {
            $('#cdRuleWvDetailTable').bootstrapTable({
                ...
                url: "${ctx}/wms/basicdata/banQinCdRuleWvDetail/data",
        ...
        function saveDetail() {
            ...
            if (detailSubmitCheck()) {
                ...
                // 保存前赋值
                beforeSave();
                jp.post("${ctx}/wms/basicdata/banQinCdRuleWvDetail/save", $('#inputForm1').bq_serialize(), function (data) {
                    if (data.success) {
        ...
        function removeDetail() {
            ...
            jp.confirm('确认要删除吗？删除将不能恢复？', function () {
                jp.get("${ctx}/wms/basicdata/banQinCdRuleWvDetail/deleteAll?ids=" + rowIds, function (data) {
                    if (data.success) {
                        $('#cdRuleWvDetailTable').bootstrapTable('refresh');
                        hideTabRight();
                        jp.success(data.msg);
        ...
        function addDetailWvRow(list, idx, tpl, row) {
            if (!row) {
                tpl = tpl.replace("{{row.orgId}}", currentRow.orgId);
        ...
        function initDetailWv() {
            ...
            params += "&orgId=" + currentRow.orgId;
            jp.post("${ctx}/wms/basicdata/banQinCdRuleWvDetailWv/data?" + params, null, function (data) {
        ...
        function saveDetailWv() {
            ...
            jp.post("${ctx}/wms/basicdata/banQinCdRuleWvDetailWv/save", $('#inputForm2').serialize(), function (data) {
                if (data.success) {
                    jp.success(data.msg);
                    hideTabRight();
                    $('#cdRuleWvDetailTable').bootstrapTable('refresh');
        ...
        function removeDetailWv() {
            ...
            jp.get("${ctx}/wms/basicdata/banQinCdRuleWvDetailWv/deleteAll?ids=" + ids.join(","), function (data) {
                if (data.success) {
                    jp.success(data.msg);
                    // 移除选中行
                    for (var i in idRows) {
        ...
        function initDetailOrder() {
            ...
            var params = "headerId=" + currentRow.id;
            params += "&orgId=" + currentRow.orgId;
            jp.post("${ctx}/wms/basicdata/banQinCdRuleWvDetailOrder/data?" + params, null, function (data) {
        ...
        function saveDetailOrder() {
            ...
            jp.loading();
            jp.post("${ctx}/wms/basicdata/banQinCdRuleWvDetailOrder/save", $('#inputForm3').serialize(), function (data) {
                if (data.success) {
                    jp.success(data.msg);
                    hideTabRight();
                    $('#cdRuleWvDetailTable').bootstrapTable('refresh');
        ...
        function removeDetailOrder() {
            ...
            jp.loading();
            jp.get("${ctx}/wms/basicdata/banQinCdRuleWvDetailOrder/deleteAll?ids=" + ids.join(","), function (data) {
                if (data.success) {
                    jp.success(data.msg);
                    // 移除选中行
                    for (var i in idRows) {
...
<div class="wrapper wrapper-content-my">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <form:form id="inputForm" modelAttribute="banQinCdRuleWvHeader" method="post" class="form-horizontal">
                <form:hidden path="id"/>
        ...
        <div class="panel-body">
            <div style="width: 100%; padding-left: 10px;">
                <div id="detailToolbar" style="width: 100%; padding: 10px 0px;">
                    <shiro:hasPermission name="basicdata:banQinCdRuleWvDetail:addDetail">
                        <a class="btn btn-primary btn-sm" id="addDetail" onclick="addDetail()">新增</a>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="basicdata:banQinCdRuleWvDetail:saveDetail">
                        <a class="btn btn-primary btn-sm" id="saveDetail" onclick="saveDetail()">保存</a>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="basicdata:banQinCdRuleWvDetail:removeDetail">
                        <a class="btn btn-danger btn-sm" id="deleteDetail" onclick="removeDetail()">删除</a>
                    </shiro:hasPermission>
                </div>
                <div id="tab-left">
                    ...
                    <div class="tabs-container" style="height: 100%; width: 100%; padding: 10px 10px;">
                        ...
                        <div class="tab-content">
                            <div id="mainRule" class="tab-pane fade in active">
                                <form:form id="inputForm1" method="post" class="form-horizontal">
                                    ...
                                    <table class="bq-table">
                                        ...
                                        <tr>
                                            ...
                                            <td class="pad" width="20%">
                                                <input id="detail_descr" name="descr" htmlEscape="false" class="form-control" maxlength="128"/>
                                            </td>
                                            <td class="pad" width="20%">
                                ...
                                <div style="width: 100%; padding-top: 10px; padding-left: 10px;">
                                    <a class="btn btn-primary btn-sm" id="addDetailWv" onclick="addDetailWv()">新增</a>
                                    <a class="btn btn-primary btn-sm" id="saveDetailWv" onclick="saveDetailWv()">保存</a>
                                    <a class="btn btn-danger btn-sm" id="removeDetailWv" onclick="removeDetailWv()">删除</a>
                                </div>
                        ...
                        <tr id="ruleWvDetailWvList{{idx}}">
                            ...
                            <td class="hide">
                                ...
                                <input id="ruleWvDetailWvList{{idx}}_orgId" name="ruleWvDetailWvList[{{idx}}].orgId" type="hidden" value="{{row.orgId}}"/>
                            ...
                            <div id="orderCondition" class="tab-pane fade">
                                <div style="width: 100%; padding-top: 10px; padding-left: 10px;">
                                    <a class="btn btn-primary btn-sm" id="addDetailOrder" onclick="addDetailOrder()">新增</a>
                                    <a class="btn btn-primary btn-sm" id="saveDetailOrder" onclick="saveDetailOrder()">保存</a>
                                    <a class="btn btn-danger btn-sm" id="removeDetailOrder" onclick="removeDetailOrder()">删除</a>
                                </div>
                        ...
                        <tr id="ruleWvDetailOrderList{{idx}}">
                            ...
                            <td class="hide">
                                ...
                                <input id="ruleWvDetailOrderList{{idx}}_orgId" name="ruleWvDetailOrderList[{{idx}}].orgId" type="hidden" value="{{row.orgId}}"/>
