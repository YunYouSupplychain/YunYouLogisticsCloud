...
function initQualificationTable() {
    ...
    $table.bootstrapTable({
        ...
        url: "${ctx}/sys/common/tms/vehicleQualification/data",
    ...
    $table.on('check.bs.table uncheck.bs.table load-success.bs.table check-all.bs.table uncheck-all.bs.table', function () {
        var ids = getCarQualificationIdSelections();
        $('#carQualification_remove').attr('disabled', !ids.length);
        $('#carQualification_edit').attr('disabled', ids.length !== 1);
    });
}
...
function initPartTable() {
    ...
    $table.bootstrapTable({
        ...
        pagination: true,// 是否显示分页
        sidePagination: "server",// client客户端分页，server服务端分页
        url: "${ctx}/sys/common/tms/vehiclePart/data",
        queryParams: function (params) {
            var searchParam = $("#searchForm").serializeJSON();
            searchParam.carNo = $("#carNo").val();
            searchParam.dataSet = $("#dataSet").val();
    ...
    $table.on('check.bs.table uncheck.bs.table load-success.bs.table check-all.bs.table uncheck-all.bs.table', function () {
        var ids = getCarPartIdSelections();
        $('#carPart_remove').attr('disabled', !ids.length);
        $('#carPart_edit').attr('disabled', ids.length !== 1);
    });
}
...
function doSubmit($table, index) {
    ...
    jp.post("${ctx}/sys/common/tms/vehicle/save", data, function (data) {
        if (data.success) {
            jp.success(data.msg);
...
function carQualificationModal(row){
	...
	$("#carQualification_dataSet").val($("#dataSet").val());
...
function saveCarQualification() {
	...
	jp.post("${ctx}/sys/common/tms/vehicleQualification/save", $('#tmVehicleQualificationForm').serialize(), function (data) {
		if (data.success) {
...
function delCarQualification() {
	jp.confirm('确认要删除该车辆资质记录吗？', function () {
		jp.loading();
		jp.get("${ctx}/sys/common/tms/vehicleQualification/deleteAll?ids=" + getCarQualificationIdSelections(), function (data) {
			if (data.success) {
				$('#tmVehicleQualificationTable').bootstrapTable('refresh');
				jp.success(data.msg);
			} else {
...
function carPart(row){
	...
	$("#carPart_dataSet").val($("#dataSet").val());
...
function saveCarPart() {
	...
	jp.post("${ctx}/sys/common/tms/vehiclePart/save", $('#tmVehiclePartForm').serialize(), function (data) {
		if (data.success) {
...
function delCarPart() {
	jp.confirm('确认要删除该车辆配件记录吗？', function () {
		jp.loading();
		jp.get("${ctx}/sys/common/tms/vehiclePart/deleteAll?ids=" + getCarPartIdSelections(), function (data) {
			if (data.success) {
				$('#tmVehiclePartTable').bootstrapTable('refresh');
				jp.success(data.msg);
			} else {
