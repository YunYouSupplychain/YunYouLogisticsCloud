    ...
    <script type="text/javascript">
        ...
        function save() {
            ...
            if (!isValidate) {
                ...
                jp.loading();
                jp.post("${ctx}/sys/common/wms/qcItemHeader/save", $('#inputForm').serialize(), function (data) {
                    if (data.success) {
                        jp.success(data.msg);
                        window.location = "${ctx}/sys/common/wms/qcItemHeader/form?id=" + data.body.entity.id;
                    } else {
                        jp.bqError(data.msg);
                    }
                })
        ...
        function initCdWhQcItemDetailTable() {
            $('#cdWhQcItemDetailTable').bootstrapTable({
                ...
                url: "${ctx}/sys/common/wms/qcItemDetail/data",
        ...
        function saveDetail() {
            ...
            if (detailSubmitCheck()) {
                ...
                // 保存前赋值
                beforeSave();
                jp.post("${ctx}/sys/common/wms/qcItemDetail/save", $('#inputForm1').serialize(), function (data) {
                    if (data.success) {
        ...
        function removeDetail() {
            ...
            jp.confirm('确认要删除吗？删除将不能恢复？', function () {
                jp.get("${ctx}/sys/common/wms/qcItemDetail/deleteAll?ids=" + rowIds, function (data) {
                    if (data.success) {
                        $('#cdWhQcItemDetailTable').bootstrapTable('refresh');
                        hideTabRight();
                        jp.success(data.msg);
...
<div class="wrapper wrapper-content-my">
    <div class="panel panel-default">
        <div class="panel-body">
            <form:form id="inputForm" modelAttribute="sysWmsQcItemHeader" method="post" class="form-horizontal">
                <form:hidden path="id"/>
                <form:hidden path="recVer"/>
                <table class="table">
                    <tbody>
                    <tr>
                        <td class="width-8"><label class="pull-right"><font color="red">*</font>质检项组编码</label></td>
                        <td class="width-12">
                            <form:input path="itemGroupCode" htmlEscape="false" class="form-control required" maxlength="32"/>
                        </td>
                        <td class="width-8"><label class="pull-right"><font color="red">*</font>质检项组名称</label></td>
                        <td class="width-12">
                        ...
                        <td class="width-12">
                            <sys:grid title="选择数据套" url="${ctx}/sys/common/dataSet/grid" cssClass="form-control required"
                                      fieldId="dataSet" fieldName="dataSet" fieldKeyName="code" fieldValue="${sysWmsQcItemHeader.dataSet}"
                                      displayFieldId="dataSetName" displayFieldName="dataSetName" displayFieldKeyName="name" displayFieldValue="${sysWmsQcItemHeader.dataSetName}"
...
<div class="wrapper wrapper-content-my">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <div id="detailToolbar" style="width: 100%; padding: 0 0 10px 0px;">
                <shiro:hasPermission name="sys:common:wms:qcItem:addDetail">
                    <a class="btn btn-primary btn-sm" id="addDetail" onclick="addDetail()">新增</a>
                </shiro:hasPermission>
                <shiro:hasPermission name="sys:common:wms:qcItem:saveDetail">
                    <a class="btn btn-primary btn-sm" id="saveDetail" onclick="saveDetail()">保存</a>
                </shiro:hasPermission>
                <shiro:hasPermission name="sys:common:wms:qcItem:removeDetail">
                    <a class="btn btn-primary btn-sm" id="deleteDetail" onclick="removeDetail()">删除</a>
                </shiro:hasPermission>
            </div>
            <div id="tab-left">
