...
function initTable() {
    ...
    $table.bootstrapTable({
        ...
        url: "${ctx}/sys/common/tms/vehicle/data",
...
function deleteAll() {
	jp.confirm('确认要删除该车辆信息记录吗？', function () {
		jp.loading();
		jp.get("${ctx}/sys/common/tms/vehicle/deleteAll?ids=" + getIdSelections(), function (data) {
			if (data.success) {
				$('#tmVehicleTable').bootstrapTable('refresh');
				jp.success(data.msg);
			} else {
...
function add() {
	jp.openDialog('车辆信息', "${ctx}/sys/common/tms/vehicle/form", '100%', '100%', $('#tmVehicleTable'));
}
...
function edit(id){
  ...
  jp.openDialog('车辆信息', "${ctx}/sys/common/tms/vehicle/form?id=" + id,'100%', '100%', $('#tmVehicleTable'));
}

function enable(flag) {
	jp.loading();
	jp.post("${ctx}/sys/common/tms/vehicle/enable?ids=" + getIdSelections() + "&flag=" + flag, {}, function (data) {
		if (data.success) {
			$('#tmVehicleTable').bootstrapTable('refresh');
			jp.success(data.msg);
		} else {
...
function updateStatus(flag) {
    jp.loading();
    jp.post("${ctx}/sys/common/tms/vehicle/updateStatus?ids=" + getIdSelections() + "&flag=" + flag, {}, function (data) {
        if (data.success) {
            $('#tmVehicleTable').bootstrapTable('refresh');
            jp.success(data.msg);
        } else {
...
function syncSelect() {
    ...
    jp.loading("同步中...");
    jp.post("${ctx}/sys/common/tms/vehicle/syncSelect", {"ids": rowIds.join(',')}, function (data) {
        jp.alert(data.msg);
    });
}
...
function syncAll() {
    jp.confirm("确认同步全部数据？", function () {
        jp.loading("同步中...");
        jp.get("${ctx}/sys/common/tms/vehicle/syncAll", function (data) {
            jp.alert(data.msg);
        });
    });
}
