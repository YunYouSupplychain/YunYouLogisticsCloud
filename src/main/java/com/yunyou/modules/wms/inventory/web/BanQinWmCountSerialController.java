 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/wms/inventory/banQinWmCountSerial")
public class BanQinWmCountSerialController extends BaseController {
    @Autowired
     ...
     * 库存序列号盘点列表页面
     */
    @RequiresPermissions("inventory:banQinWmCountSerial:list")
    @RequestMapping(value = {"list", ""})
    public String list() {
        return "modules/wms/inventory/banQinWmCountSerialList";
    }
     ...
     * 查看，增加，编辑库存序列号盘点表单页面
     */
    @RequiresPermissions(value = {"inventory:banQinWmCountSerial:view", "inventory:banQinWmCountSerial:add", "inventory:banQinWmCountSerial:edit"}, logical = Logical.OR)
    ...
    public String form(BanQinWmCountHeaderEntity banQinWmCountHeaderEntity, Model model) {
        ...
        return "modules/wms/inventory/banQinWmCountSerialForm";
    }
     ...
     */
    @ResponseBody
    @RequiresPermissions(value = {"inventory:banQinWmCountSerial:add", "inventory:banQinWmCountSerial:edit"}, logical = Logical.OR)
    ...
    public AjaxJson save(BanQinWmCountHeaderEntity banQinWmCountHeaderEntity, Model model, RedirectAttributes redirectAttributes) throws Exception {
        ...
        try {
            BanQinWmCountHeaderEntity entity = banQinWmCountHeaderService.saveSerialEntity(banQinWmCountHeaderEntity);
            j.setMsg("保存成功");
     ...
     */
    @ResponseBody
    @RequiresPermissions("inventory:banQinWmCountSerial:del")
     ...
     */
    @ResponseBody
    @RequiresPermissions("inventory:banQinWmCountSerial:export")
    @RequestMapping(value = "export", method = RequestMethod.POST)
    public AjaxJson exportFile(BanQinWmCountHeader banQinWmCountHeader, HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes) {
        AjaxJson j = new AjaxJson();
        try {
            String fileName = "库存盘点" + DateUtils.getDate("yyyyMMddHHmmss") + ".xlsx";
            Page<BanQinWmCountHeader> page = banQinWmCountHeaderService.findPage(new Page<BanQinWmCountHeader>(request, response, -1), banQinWmCountHeader);
            new ExportExcel("库存盘点", BanQinWmCountHeader.class).setDataList(page.getList()).write(response, fileName).dispose();
            j.setSuccess(true);
            j.setMsg("导出成功！");
            return j;
        } catch (Exception e) {
     ...
     * 导入Excel数据
     */
    @RequiresPermissions("inventory:banQinWmCountSerial:import")
    ...
    public String importFile(MultipartFile file, RedirectAttributes redirectAttributes) {
        ...
        return "redirect:" + Global.getAdminPath() + "/wms/inventory/banQinWmCountSerial/?repage";
    }
