 ...
 */
@Service
public class BmsWaybillBusinessService extends BaseService {
    @Autowired
     ...
     * @return 原始业务数据
     */
    private List<BmsWaybillData> findBusinessData(List<BmsCalcTermsParams> includeParams, List<BmsCalcTermsParams> excludeParams) {
        return bmsWaybillDataService.findCalcData(includeParams, excludeParams);
    }
     ...
     * @return 商品参数
     */
    private List<BmsBusinessSkuParams> findSkuParams(List<BmsWaybillData> list, String orgId) {
    ...
    public List<BmsCalcBusinessData> findForCalcOutbound(String outputObject, List<BmsCalcTermsParams> includeParams, List<BmsCalcTermsParams> excludeParams, String orgId) {
        List<BmsWaybillData> data = this.findBusinessData(includeParams, excludeParams);
        ...
        switch (outputObject) {
            case BmsConstants.OUTPUT_OBJECT_001:
                return data.stream().collect(Collectors.groupingBy(o -> o.getOrderNo() + o.getOrgId())).values().stream().map(list -> {
                    ...
                    bmsBusinessParams.setBusinessDataIds(list.stream().map(BmsWaybillData::getId).collect(Collectors.toList()));
