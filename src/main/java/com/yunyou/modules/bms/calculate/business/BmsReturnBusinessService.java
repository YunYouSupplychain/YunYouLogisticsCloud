 ...
 */
@Service
public class BmsReturnBusinessService extends BaseService {
    @Autowired
     ...
     * @return 原始业务数据
     */
    private List<BmsReturnData> findBusinessData(List<BmsCalcTermsParams> includeParams, List<BmsCalcTermsParams> excludeParams) {
        return bmsReturnDataService.findCalcData(includeParams, excludeParams);
    }
     ...
     * @return 商品参数
     */
    private List<BmsBusinessSkuParams> findSkuParams(List<BmsReturnData> list, String orgId) {
        List<BmsBusinessSkuParams> rsList = Lists.newArrayList();
        for (BmsReturnData o : list) {
     ...
     * @return 业务数据
     */
    public List<BmsCalcBusinessData> findForCalcReturn(String outputObject, List<BmsCalcTermsParams> includeParams, List<BmsCalcTermsParams> excludeParams, String orgId) {
        List<BmsReturnData> data = this.findBusinessData(includeParams, excludeParams);
        ...
        switch (outputObject) {
            case BmsConstants.OUTPUT_OBJECT_001:
                return data.stream().collect(Collectors.groupingBy(o -> o.getOrderNo() + o.getOrgId())).values().stream().map(list -> {
                    ...
                    bmsBusinessParams.setBusinessDataIds(list.stream().map(BmsReturnData::getId).collect(Collectors.toList()));
