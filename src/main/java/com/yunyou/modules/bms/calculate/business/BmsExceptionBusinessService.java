 ...
 */
@Service
public class BmsExceptionBusinessService extends BaseService {
    @Autowired
     ...
     * @return 原始业务数据
     */
    private List<BmsExceptionData> findBusinessData(List<BmsCalcTermsParams> includeParams, List<BmsCalcTermsParams> excludeParams) {
        return bmsExceptionDataService.findCalcData(includeParams, excludeParams);
    }
     ...
     * @return 商品参数
     */
    private List<BmsBusinessSkuParams> findSkuParams(List<BmsExceptionData> list, String orgId) {
        List<BmsBusinessSkuParams> rsList = Lists.newArrayList();
        for (BmsExceptionData o : list) {
     ...
     * @return 业务数据
     */
    public List<BmsCalcBusinessData> findForCalcException(String outputObject, List<BmsCalcTermsParams> includeParams, List<BmsCalcTermsParams> excludeParams, String orgId) {
        List<BmsExceptionData> data = this.findBusinessData(includeParams, excludeParams);
        ...
        switch (outputObject) {
            case BmsConstants.OUTPUT_OBJECT_001:
                return data.stream().collect(Collectors.groupingBy(o -> o.getOrderNo() + o.getOrgId())).values().stream().map(list -> {
                    ...
                    bmsBusinessParams.setCarTypeCode(o.getCarType());
                    ...
                    bmsBusinessParams.setBusinessDataIds(list.stream().map(BmsExceptionData::getId).collect(Collectors.toList()));
