...
/**
 * 描述：BMS拉取退货数据定时任务(DisallowConcurrentExecution:禁止并发执行多个相同定义的JobDetail)
 *
 * @author Jianhua
 * @version 2019/6/4
 */
@DisallowConcurrentExecution
public class BmsPullReturnDataTask extends Task {
    @Override
    public void run() {
        Date date = DateUtil.subMonths(new Date(), 1);// 上个月
        SpringContextHolder.getBean(BmsPullDataHandle.class).dataHandle(new BmsPullDataCondition(BmsPullDataCondition.DATA_TYPE_RETURN, DateUtil.beginOfMonth(date), DateUtil.endOfMonth(date)));
    }
}
