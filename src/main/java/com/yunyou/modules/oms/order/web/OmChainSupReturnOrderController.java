 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/oms/order/omChainSupReturnOrder")
public class OmChainSupReturnOrderController extends BaseController {
    @Autowired
     ...
     * 供应商退货订单列表页面
     */
    @RequiresPermissions("order:omChainSupReturnOrder:list")
    @RequestMapping(value = {"list", ""})
    public String list() {
        return "modules/oms/order/omChainSupReturnOrderList";
    }
    ...
    public Map<String, Object> data(OmChainHeaderEntity omChainHeaderEntity, HttpServletRequest request, HttpServletResponse response) {
        ...
        omChainHeaderEntity.setBusinessOrderTypeList(
                Lists.newArrayList(BusinessOrderType.SUP_MRB.getBusinessType()));
     ...
     * 查看，增加，编辑供应商退货订单表单页面
     */
    @RequiresPermissions(value = {"order:omChainSupReturnOrder:view", "order:omChainSupReturnOrder:add", "order:omChainSupReturnOrder:edit"}, logical = Logical.OR)
    ...
    public String form(OmChainHeaderEntity omChainHeaderEntity, Model model) {
        ...
        return "modules/oms/order/omChainSupReturnOrderForm";
    }
     ...
     */
    @ResponseBody
    @RequiresPermissions(value = {"order:omChainSupReturnOrder:add", "order:omChainSupReturnOrder:edit"}, logical = Logical.OR)
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainSupReturnOrder:del")
    ...
    public AjaxJson exportFile(OmChainHeaderEntity entity, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            j.setSuccess(false);
            j.setMsg("导出供应商退货订单记录失败！失败信息：" + e.getMessage());
        }
        return j;
    }
     ...
     * 导入Excel数据
     */
    @RequiresPermissions("order:omChainSupReturnOrder:import")
    ...
    public String importFileTemplate(HttpServletResponse response, RedirectAttributes redirectAttributes) {
        try {
            new ExportExcel("", OmChainImportEntity.class, 2).setDataList(Lists.newArrayList()).write(response, "供应商退货订单导入模板.xlsx").dispose();
            return null;
        } catch (Exception e) {
            addMessage(redirectAttributes, "导入模板下载失败！失败信息：" + e.getMessage());
        }
        return "redirect:" + Global.getAdminPath() + "/oms/order/omChainSupReturnOrder/?repage";
    }
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainSupReturnOrder:audit")
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainSupReturnOrder:cancelAudit")
     ...
     */
    @RequiresPermissions("order:omChainSupReturnOrder:cancel")
    @ResponseBody
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainSupReturnOrder:intercept")
