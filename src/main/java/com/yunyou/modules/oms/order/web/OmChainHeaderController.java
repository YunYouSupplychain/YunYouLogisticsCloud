 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/oms/order/omChainHeader")
public class OmChainHeaderController extends BaseController {
    @Autowired
     ...
     * 供应链订单列表页面
     */
    @RequiresPermissions("order:omChainHeader:list")
    @RequestMapping(value = {"list", ""})
    public String list() {
        return "modules/oms/order/omChainHeaderList";
    }
     ...
     * 查看，增加，编辑供应链订单表单页面
     */
    @RequiresPermissions(value = {"order:omChainHeader:view", "order:omChainHeader:add", "order:omChainHeader:edit"}, logical = Logical.OR)
    ...
    public String form(OmChainHeaderEntity omChainHeaderEntity, Model model) {
        ...
        return "modules/oms/order/omChainHeaderForm";
    }
     ...
     */
    @ResponseBody
    @RequiresPermissions(value = {"order:omChainHeader:add", "order:omChainHeader:edit"}, logical = Logical.OR)
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainHeader:del")
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainHeader:export")
    ...
    public AjaxJson exportFile(OmChainHeaderEntity entity, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            new ExportExcel("", OmChainHeaderExportEntity.class).setDataList(page.getList()).write(response, "供应链订单.xlsx").dispose();
            j.setSuccess(true);
            j.setMsg("导出成功！");
            return j;
        } catch (Exception e) {
            j.setSuccess(false);
            j.setMsg("导出供应链订单记录失败！失败信息：" + e.getMessage());
        }
        return j;
    }
     ...
     * 导入Excel数据
     */
    @RequiresPermissions("order:omChainHeader:import")
    ...
    public String importFile(MultipartFile file, String orgId, RedirectAttributes redirectAttributes) {
        ...
        return "redirect:" + Global.getAdminPath() + "/oms/order/omChainHeader/?repage";
    }
    ...
    public String importFileTemplate(HttpServletResponse response, RedirectAttributes redirectAttributes) {
        try {
            new ExportExcel("", OmChainImportEntity.class, 2).setDataList(Lists.newArrayList()).write(response, "供应链订单导入模板.xlsx").dispose();
            return null;
        } catch (Exception e) {
            addMessage(redirectAttributes, "导入模板下载失败！失败信息：" + e.getMessage());
        }
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainHeader:audit")
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainHeader:cancelAudit")
     ...
     */
    @RequiresPermissions("order:omChainHeader:cancel")
    @ResponseBody
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainHeader:intercept")
