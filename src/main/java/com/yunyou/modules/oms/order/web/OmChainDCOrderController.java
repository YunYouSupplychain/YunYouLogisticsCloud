 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/oms/order/omChainDCOrder")
public class OmChainDCOrderController extends BaseController {
    @Autowired
     ...
     * DC订单列表页面
     */
    @RequiresPermissions("order:omChainDCOrder:list")
    @RequestMapping(value = {"list", ""})
    public String list() {
        return "modules/oms/order/omChainDCOrderList";
    }
    ...
    public Map<String, Object> data(OmChainHeaderEntity omChainHeaderEntity, HttpServletRequest request, HttpServletResponse response) {
        ...
        omChainHeaderEntity.setBusinessOrderTypeList(
                Lists.newArrayList(BusinessOrderType.DC_ASN.getBusinessType(), BusinessOrderType.DC_SO.getBusinessType()));
     ...
     * 查看，增加，编辑DC订单表单页面
     */
    @RequiresPermissions(value = {"order:omChainDCOrder:view", "order:omChainDCOrder:add", "order:omChainDCOrder:edit"}, logical = Logical.OR)
    ...
    public String form(OmChainHeaderEntity omChainHeaderEntity, Model model) {
        ...
        return "modules/oms/order/omChainDCOrderForm";
    }
     ...
     */
    @ResponseBody
    @RequiresPermissions(value = {"order:omChainDCOrder:add", "order:omChainDCOrder:edit"}, logical = Logical.OR)
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainDCOrder:del")
    ...
    public AjaxJson exportFile(OmChainHeaderEntity entity, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            j.setSuccess(false);
            j.setMsg("导出DC订单记录失败！失败信息：" + e.getMessage());
        }
        return j;
    }
     ...
     * 导入Excel数据
     */
    @RequiresPermissions("order:omChainDCOrder:dc:importINT")
    ...
    public String importFile(MultipartFile file, String orgId, RedirectAttributes redirectAttributes) {
        try {
            ...
            if (CollectionUtil.isEmpty(list)) {
                throw new OmsException("EXCEL数据为空！");
            }
            list.forEach(v -> {
                v.setOrgId(orgId);
                v.setOrderSource(OrderSource.INT.getCode());
                v.setSourceOrderNo(file.getOriginalFilename());
            });
            ResultMessage msg = omChainHeaderService.importDCOrderByBusinessType(list, BusinessOrderType.DC_ASN, BusinessOrderType.DC_SO, OmsConstants.IMPORT_TYPE_DC_INT);
            if (!msg.isSuccess()) {
                throw new OmsException(msg.getMessage());
            }
            addMessage(redirectAttributes, "导入成功");
        } catch (Exception e) {
            addMessage(redirectAttributes, "导入模板下载失败！失败信息：" + e.getMessage());
        }
        return "redirect:" + Global.getAdminPath() + "/oms/order/omChainDCOrder/?repage";
    }
    ...
    public String importFileTemplate(HttpServletResponse response, RedirectAttributes redirectAttributes) {
        try {
            new ExportExcel("", OmChainDCImportEntity.class, 2).setDataList(Lists.newArrayList()).write(response, "DC订单导入模板.xlsx").dispose();
            return null;
        } catch (Exception e) {
            addMessage(redirectAttributes, "导入模板下载失败！失败信息：" + e.getMessage());
        }
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainDCOrder:audit")
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainDCOrder:cancelAudit")
     ...
     */
    @RequiresPermissions("order:omChainDCOrder:cancel")
    @ResponseBody
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainDCOrder:intercept")
