 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/oms/order/omChainContractLogisticsOrder")
public class OmChainContractLogisticsOrderController extends BaseController {
    @Autowired
     ...
     * 合同物流订单列表页面
     */
    @RequiresPermissions("order:omChainContractLogisticsOrder:list")
    @RequestMapping(value = {"list", ""})
    public String list() {
        return "modules/oms/order/omChainContractLogisticsOrderList";
    }
    ...
    public Map<String, Object> data(OmChainHeaderEntity omChainHeaderEntity, HttpServletRequest request, HttpServletResponse response) {
        ...
        omChainHeaderEntity.setBusinessOrderTypeList(
                Lists.newArrayList(BusinessOrderType.CONTRACT_LOGISTICS_ASN.getBusinessType(), BusinessOrderType.CONTRACT_LOGISTICS_SO.getBusinessType()));
     ...
     * 查看，增加，编辑合同物流订单表单页面
     */
    @RequiresPermissions(value = {"order:omChainContractLogisticsOrder:view", "order:omChainContractLogisticsOrder:add", "order:omChainContractLogisticsOrder:edit"}, logical = Logical.OR)
    ...
    public String form(OmChainHeaderEntity omChainHeaderEntity, Model model) {
        ...
        return "modules/oms/order/omChainContractLogisticsOrderForm";
    }
     ...
     */
    @ResponseBody
    @RequiresPermissions(value = {"order:omChainContractLogisticsOrder:add", "order:omChainContractLogisticsOrder:edit"}, logical = Logical.OR)
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainContractLogisticsOrder:del")
    ...
    public AjaxJson exportFile(OmChainHeaderEntity entity, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            j.setSuccess(false);
            j.setMsg("导出合同物流订单记录失败！失败信息：" + e.getMessage());
        }
        return j;
    }
     ...
     * 导入Excel数据
     */
    @RequiresPermissions("order:omChainContractLogisticsOrder:import")
    ...
    public String importFileTemplate(HttpServletResponse response, RedirectAttributes redirectAttributes) {
        try {
            new ExportExcel("", OmChainImportEntity.class, 2).setDataList(Lists.newArrayList()).write(response, "合同物流订单导入模板.xlsx").dispose();
            return null;
        } catch (Exception e) {
            addMessage(redirectAttributes, "导入模板下载失败！失败信息：" + e.getMessage());
        }
        return "redirect:" + Global.getAdminPath() + "/oms/order/omChainContractLogisticsOrder/?repage";
    }
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainContractLogisticsOrder:audit")
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainContractLogisticsOrder:cancelAudit")
     ...
     */
    @RequiresPermissions("order:omChainContractLogisticsOrder:cancel")
    @ResponseBody
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainContractLogisticsOrder:intercept")
