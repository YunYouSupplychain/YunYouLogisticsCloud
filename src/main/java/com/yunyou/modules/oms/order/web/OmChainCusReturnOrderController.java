 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/oms/order/omChainCusReturnOrder")
public class OmChainCusReturnOrderController extends BaseController {
    @Autowired
     ...
     * 客户退货订单列表页面
     */
    @RequiresPermissions("order:omChainCusReturnOrder:list")
    @RequestMapping(value = {"list", ""})
    public String list() {
        return "modules/oms/order/omChainCusReturnOrderList";
    }
    ...
    public Map<String, Object> data(OmChainHeaderEntity omChainHeaderEntity, HttpServletRequest request, HttpServletResponse response) {
        ...
        omChainHeaderEntity.setBusinessOrderTypeList(
                Lists.newArrayList(BusinessOrderType.CUS_MRB.getBusinessType()));
     ...
     * 查看，增加，编辑客户退货订单表单页面
     */
    @RequiresPermissions(value = {"order:omChainCusReturnOrder:view", "order:omChainCusReturnOrder:add", "order:omChainCusReturnOrder:edit"}, logical = Logical.OR)
    ...
    public String form(OmChainHeaderEntity omChainHeaderEntity, Model model) {
        ...
        return "modules/oms/order/omChainCusReturnOrderForm";
    }
     ...
     */
    @ResponseBody
    @RequiresPermissions(value = {"order:omChainCusReturnOrder:add", "order:omChainCusReturnOrder:edit"}, logical = Logical.OR)
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainCusReturnOrder:del")
    ...
    public AjaxJson exportFile(OmChainHeaderEntity entity, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            j.setSuccess(false);
            j.setMsg("导出客户退货订单记录失败！失败信息：" + e.getMessage());
        }
        return j;
    }
     ...
     * 导入Excel数据
     */
    @RequiresPermissions("order:omChainCusReturnOrder:import")
    ...
    public String importFileTemplate(HttpServletResponse response, RedirectAttributes redirectAttributes) {
        try {
            new ExportExcel("", OmChainImportEntity.class, 2).setDataList(Lists.newArrayList()).write(response, "客户退货订单导入模板.xlsx").dispose();
            return null;
        } catch (Exception e) {
            addMessage(redirectAttributes, "导入模板下载失败！失败信息：" + e.getMessage());
        }
        return "redirect:" + Global.getAdminPath() + "/oms/order/omChainCusReturnOrder/?repage";
    }
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainCusReturnOrder:audit")
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainCusReturnOrder:cancelAudit")
     ...
     */
    @RequiresPermissions("order:omChainCusReturnOrder:cancel")
    @ResponseBody
     ...
     */
    @ResponseBody
    @RequiresPermissions("order:omChainCusReturnOrder:intercept")
