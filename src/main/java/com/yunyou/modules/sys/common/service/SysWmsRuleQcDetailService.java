...
import com.yunyou.modules.sys.common.mapper.SysWmsRuleQcDetailMapper;
...
public class SysWmsRuleQcDetailService extends CrudService<SysWmsRuleQcDetailMapper, SysWmsRuleQcDetail> {
    @Autowired
    ...
    @Override
    @Transactional
    public void save(SysWmsRuleQcDetail entity) {
        if (StringUtils.isEmpty(entity.getId())) {
            entity.setLineNo(wmsUtil.getMaxLineNo("sys_wms_rule_qc_detail", "header_id", entity.getHeaderId()));
        }
        super.save(entity);
    ...
    @Override
    @Transactional
    public void delete(SysWmsRuleQcDetail entity) {
        super.delete(entity);
        final String IS_SAVING_PUSH = SysControlParamsUtils.getValue(SysParamConstants.IS_SAVING_PUSH);
        if ("Y".equals(IS_SAVING_PUSH)) {
            sysWmsRuleQcHeaderService.findSync(new SysWmsRuleQcHeader(entity.getHeaderId())).forEach(syncPlatformDataToWmsAction::sync);
